...
/**
 * Created by taylor.yan on 3/7/17.
 */

public class SlideBar extends View {

    //定义了显示在最右边的浮动的索引项的列表,当然这个是固定的，所以可以直接初始化，如果需要变动的话则可以通过自定义属性来指定
    public String[] mIndexStrings = {  "#","A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K",
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int singleHeight = height / mIndexStrings.length;
        //遍历循环绘制每一个字母text
        for (int i = 0; i < mIndexStrings.length; i++) {
             ...
             *xPos = width / 2 - paint.measureText(mIndexStrings[i]) / 2:意思很容易理解,就是用
             * (总的view的宽度(可能还包括leftPadding和rightPadding的大小)-每个字母宽度)/2得到就是每个字母的左下角的X坐标,
             * 仔细想下每个text的起点的x坐标都是一样的.paint.measureText(mIndexStrings[i])得到每一个字母宽度大小
             * 由于是左下角，所以它们的Y坐标:应该如下设置 yPos = singleHeight * i + singleHeight;
             * */
            float xPos = width / 2 - paint.measureText(mIndexStrings[i]) / 2;//得到绘制字母text的起点的X坐标
            float yPos = singleHeight * i + singleHeight;//得到绘制字母text的起点的Y坐标
            canvas.drawText(mIndexStrings[i], xPos, yPos, paint);//开始绘制每个字母
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {//重写view的触摸事件分发方法
        ...
        final int c = (int) (y / getHeight() * mIndexStrings.length);//得到点击或触摸的位置从而确定对应点击或触摸的字母所在字母数组中的下标
        switch (action) {
            case MotionEvent.ACTION_DOWN://监听按下事件
                ...
                if (oldChoose != c && listener != null) {//如果此次点击的字母数组下标不等于上一次的且已经注册了监听事件的,
                    if (c >= 0 && c <= mIndexStrings.length) {//并且点击得到数组下标在字母数组范围内的，我们就将此时的字母回调出去
                        listener.onTouchingLetterChanged(mIndexStrings[c]);//我们就将此时的对应在字母数组中的字母回调出去
            ...
            case MotionEvent.ACTION_MOVE://监听移动事件,因为按下的时候已经把背景showBkg设置true,这里就不需要重新设置,其他操作与按下的事件一致
                if (oldChoose != c && listener != null) {
                    if (c >= 0 && c <= mIndexStrings.length) {
                        listener.onTouchingLetterChanged(mIndexStrings[c]);
                        choose = c;
                        invalidate();
                    }
                }
